...
4. mandare a Forvo solo il residuo tramite la logica Anki/addon: sessione browser
   valida, pagina Forvo renderizzata, candidati audio estratti da `Play(...)`,
   ranking speaker, download diretto e conversione OGG -> MP3 quando serve;
...
Il fetch Forvo standard non e' scraping HTTP puro e non parte da `curl`: segue la
logica osservata nell'addon Anki, usando una sessione browser valida per superare
cookie/login/Cloudflare nel modo previsto dal sito e poi leggendo i candidati dal
player. Il riuso interno resta obbligatorio prima di aprire Forvo.
...
- aprire la pagina `https://forvo.com/word/<word>/#ja` in una sessione browser
  Forvo valida;
- attendere la pagina renderizzata;
...
Questo percorso puo richiedere una sessione browser persistente e login/cookie
gia validi. Non sostituirlo con `curl`, scraping HTTP puro o Playwright headless
per batch reali: Cloudflare e le challenge rendono quel percorso non affidabile.
...
- Non usare `curl` o scraping HTTP puro come workflow Forvo.
