...
| Local setup, environment, QA, or agent tooling | Update the related operational docs in the same slice and keep commands behind `./scripts/with-node.sh`. | `./scripts/with-node.sh pnpm format:check` plus the smallest targeted tooling test. | `docs/dev-tooling.md`, `.codex/README.md`, `.env.example` |
