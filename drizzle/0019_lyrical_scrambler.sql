CREATE TABLE `kanji_clash_pair_log` (
	...
	FOREIGN KEY (`pair_key`) REFERENCES `kanji_clash_pair_state`(`pair_key`) ON UPDATE no action ON DELETE no action
);
...
CREATE INDEX `kanji_clash_pair_log_mode_answered_idx` ON `kanji_clash_pair_log` (`mode`,`answered_at`);--> statement-breakpoint
...
CREATE INDEX `kanji_clash_pair_state_interaction_idx` ON `kanji_clash_pair_state` (`last_interaction_at`);