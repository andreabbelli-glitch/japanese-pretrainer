...
describe("kanji clash page data", () => {
  ...
  it("surfaces suspended manual contrasts in page data without queueing them", async () => {
    const baseline = await getKanjiClashPageData(
      {},
      database,
      new Date("2026-04-09T12:00:00.000Z")
    ...
    expect(
      data.queue.rounds.some(
        (round) =>
          round.origin?.type === "manual-contrast" &&
          round.origin.contrastKey === contrastKey
      )
    ).toBe(false);
  });
});
