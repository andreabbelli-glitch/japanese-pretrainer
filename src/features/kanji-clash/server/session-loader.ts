...
import { buildKanjiClashQueueSnapshot } from "../model/queue.ts";
import { generateKanjiClashCandidates } from "../model/pairing.ts";
import type {
  KanjiClashPairState,
...
async function loadAutomaticKanjiClashQueueSnapshot(input: {
  ...
  const introducedTodayCountPromise =
    countKanjiClashAutomaticNewPairIntroductions({
