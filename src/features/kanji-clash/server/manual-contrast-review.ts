...
import { listReviewCardIdsByEntryRefs } from "@/db/queries";
...
async function upsertForcedManualContrast(input: {
  ...
  const [subjectA, subjectB] = [input.current, input.target].sort(
    (left, right) => left.subjectKey.localeCompare(right.subjectKey)
  );
...
async function upsertManualContrastRoundState(input: {
  ...
  const roundKey = buildManualContrastRoundKey(
    input.contrastKey,
    input.direction
  );
  const existing =
