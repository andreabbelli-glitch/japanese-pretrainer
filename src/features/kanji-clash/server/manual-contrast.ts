...
export async function loadKanjiClashManualContrastPageSnapshot(input: {
  ...
  return {
    ...
    manualContrasts: buildKanjiClashManualContrastSummaries(rows, subjectsByKey)
  };
}
...
export async function restoreKanjiClashManualContrast(input: {
  ...
  await database.transaction(async (tx) => {
    ...
    await tx
      ...
      .where(
        eq(kanjiClashManualContrastRoundState.contrastKey, input.contrastKey)
      );
  });
}
...
export async function applyKanjiClashManualContrastRoundAnswer(input: {
  ...
  await input.transaction.insert(kanjiClashManualContrastRoundLog).values({
    ...
    direction: getManualContrastDirectionFromRoundKey(
      input.currentRound.roundKey
    ),
...
async function loadManualContrastSubjects(input: {
  ...
  const termEntryIds = parsedKeys
    .filter(
      (
        parsed
      ): parsed is { entryId: string; entryType: "term"; kind: "entry" } =>
  ...
  const [termRows, grammarRows, termGroupRows, grammarGroupRows, cardRows] =
    await Promise.all([
  ...
  for (const [groupId, rows] of groupBy(
    ...
    subjects.set(
      ...
      buildManualContrastSubject({
        ...
        reading:
          rows.find((row) => Boolean(row.reading?.trim()))?.reading ?? null,
        source: {
