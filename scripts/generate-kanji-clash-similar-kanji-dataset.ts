...
const SIMILAR_KANJI_DATASET_OUTPUT = path.resolve(
  process.cwd(),
  "src/lib/kanji-clash/similar-kanji-dataset.generated.ts"
);
...
async function main() {
  ...
  await writeFile(
    ...
    `${buildGeneratedFileBanner()}\nimport type { SimilarKanjiDataset } from "./similar-kanji-dataset-builder.ts";\n\nexport const KANJI_CLASH_SIMILAR_KANJI_DATASET: SimilarKanjiDataset = ${JSON.stringify(
      dataset,
      null,
      2
    )};\n`,
...
function buildGeneratedFileBanner() {
  return [
