...
title: "Menu e sistema: verbi che guidano l'azione"
order: 64
...
prerequisites: []
summary: >-
  Leggere conferme, ritorni, bersagli, percorsi e proposte capendo quale azione
  concreta il gioco chiede sullo schermo.
---

# Menu e sistema: verbi che guidano l'azione

Nei menu, nei tutorial e nei dialoghi brevi di *Pokémon Scarlet & Violet*, una riga di giapponese spesso vale come istruzione immediata. Il gioco ti dice se devi controllare una scelta, tornare indietro, annullare un'azione, completare una prova, fissare un bersaglio o dirigerti verso un punto della mappa.

Questi verbi non sono intercambiabili. [{{確認|かくにん}}する](term:term-e050-kakunin-suru) tiene l'attenzione sulla verifica, [もどる](term:term-e051-modoru) sposta indietro la schermata, [{{向|む}}かう](term:term-v358-muka-u) orienta il movimento e [やってみよう](term:term-e070-yattemiyou) trasforma un'azione in proposta condivisa. Quando riconosci il ruolo di ogni verbo, la UI smette di sembrare una serie di "ok" generici e diventa una mappa di azioni concrete.
...
## 1. Prompt di sistema: verificare, tornare, annullare, completare
...
*   `{{確認|かくにん}}する` ➔ **Controllo attivo** (`{{確認|かくにん}}` verifica + `する` fare ➔ controllare / confermare).
...
*   `{{前|まえ}}の メニューに` ➔ **Destinazione del ritorno** (`に` marca il menu precedente come punto di arrivo).
*   `もどる` ➔ **Movimento indietro** (la frase non conferma una scelta: riporta alla schermata prima).
...
## 2. Scelta e bersaglio: selezionare, agganciare, salvare un preferito
...
*   `{{選択|せんたく}}する` ➔ **Nome + `する`** (`{{選択|せんたく}}` selezione diventa l'azione "selezionare").

:::example_sentence
jp: >-
  ポケモンを ロックオンする。
translation_it: >-
  Aggancia un Pokémon come bersaglio.
:::
...
## 3. Azioni richieste dai personaggi: raccogliere, affidare, spiegare, sapere
...
*   `{{材料|ざいりょう}}を` ➔ **Oggetto raccolto** (i materiali sono ciò che va messo insieme).
*   `{{集|あつ}}める` ➔ **Accumulo intenzionale** (sparso ➔ riunito in un gruppo utile).

:::example_sentence
jp: >-
  これを {{頼|たの}}むよ。
translation_it: >-
  Ti affido questo, mi raccomando.
:::

#### 🗺️ Anatomia della frase

*   `これを` ➔ **Oggetto affidato** (questo è ciò di cui l'interlocutore dovrà occuparsi).
*   `{{頼|たの}}むよ` ➔ **Richiesta personale** (`よ` dà alla frase un tono diretto e coinvolto).
...
*   `やり{{方|かた}}を` ➔ **Contenuto spiegato** (non una cosa fisica, ma il metodo).
...
## 4. Movimento e tempo: verso il centro, a piedi, di corsa, entro un limite
...
*   `{{向|む}}かう` ➔ **Movimento orientato** (il focus è andare verso quel punto, non solo essere lì).
...
*   `{{全力|ぜんりょく}}で` ➔ **Modo dell'azione** (`で` indica "con / usando piena forza").
*   `{{走|はし}}る` ➔ **Movimento rapido a piedi** (correre, non cavalcare o teletrasportarsi).
...
*   `バトルの {{途中|とちゅう}}で` ➔ **Fase intermedia** (`で` colloca l'informazione durante lo svolgimento della battaglia).
...
## 5. Etichette e proposte: risorsa, leggenda, tema già nominato, piano previsto
...
*   `この ポケモンは` ➔ **Tema proposto** (`は` mette questo Pokémon davanti all'interlocutore).
...
*   `{{一回|いっかい}}` ➔ **Quantità del tentativo** (una volta, come prova limitata).
*   `やってみよう` ➔ **Invito al tentativo** (`やって` fare + `みよう` proviamo a vedere).
...
*   `ジムなら` ➔ **Tema condizionale** (se il tema è la palestra / se parli della palestra).
*   `あっちだ` ➔ **Risposta direzionale** (è da quella parte).
...
*   `{{明日|あした}}は` ➔ **Cornice temporale** (domani è il quadro in cui vale il piano).
...
Le forme diventano più solide quando si ricombinano in comandi, percorsi e dialoghi brevi:
...
:::example_sentence
jp: >-
  お{{気|き}}に{{入|い}}りの ポケモンを {{選択|せんたく}}する。
translation_it: >-
  Seleziono un Pokémon tra i preferiti.
:::

:::example_sentence
jp: >-
  {{人々|ひとびと}}の {{話|はなし}}を {{聞|き}}いて、{{中央|ちゅうおう}}へ {{向|む}}かう。
translation_it: >-
  Ascolto ciò che dice la gente e mi dirigo verso il centro.
:::

:::example_sentence
jp: >-
  {{伝説|でんせつ}}のポケモンなら、{{全力|ぜんりょく}}で {{走|はし}}ってみよう。
translation_it: >-
  Se parli del Pokémon leggendario, proviamo a correre con tutte le forze.
:::

:::example_sentence
jp: >-
  {{途中|とちゅう}}で キャンセルして、{{前|まえ}}の メニューに もどる。
translation_it: >-
  Annulla a metà e torna al menu precedente.
:::
...
Quando una schermata nuova sembra piena di comandi brevi, cerca prima il verbo che decide l'azione. [{{確認|かくにん}}する](term:term-e050-kakunin-suru) controlla, [もどる](term:term-e051-modoru) torna indietro, [キャンセル](term:term-v371-kyanseru) annulla, [クリアする](term:term-e052-kuria-suru) completa, [{{向|む}}かう](term:term-v358-muka-u) orienta il movimento e [{{選択|せんたく}}](term:term-v368-sentaku) marca la scelta. Nei dialoghi, la stessa precisione ti fa distinguere favore, proposta e piano: qualcuno può [{{頼|たの}}む](term:term-v356-tanomu), chiedere [〜は どうだ？](term:term-e069-ha-douda), suggerire [やってみよう](term:term-e070-yattemiyou) o parlare di ciò che è già [～{{予定|よてい}}だ](grammar:grammar-g073-essere-previsto).
