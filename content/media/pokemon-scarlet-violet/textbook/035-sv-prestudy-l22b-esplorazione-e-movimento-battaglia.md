---
id: lesson-pokemon-scarlet-violet-sv-prestudy-035-sv-prestudy-l22b-esplorazione-e-movimento-battaglia
media_id: media-pokemon-scarlet-violet
slug: 035-sv-prestudy-l22b-esplorazione-e-movimento-battaglia
title: "Esplorazione e sfide: muoversi, decidere e confermare"
order: 65
...
In *Pokémon Scarlet & Violet*, molte schermate sembrano semplici istruzioni di movimento, ma in realtà ti stanno già dicendo come leggere la situazione: che cosa hai intorno, quanto devi avvicinarti, se una zona è rischiosa, se una sfida sta per partire o se il sistema vuole una conferma prima di procedere.

La stessa lingua collega il campo aperto, il tutorial della cavalcatura, la battaglia e i dialoghi di sfida. Quando riconosci parole come [{{周|まわ}}り](term:term-v379-mawari), [{{近|ちか}}づく](term:term-v380-chikazu-ku), [{{危|あぶ}}ない](term:term-v389-abunai) o [{{本当|ほんとう}}に よろしいですか？](term:term-e074-hontouni-yoroshiidesuka), non stai solo traducendo: stai capendo se il gioco descrive un ambiente, chiede un'azione, segnala un pericolo o blocca tutto per una scelta consapevole.
...
*   `{{広|ひろ}}い {{水上|すいじょう}}を` ➔ **Area attraversata**: `を` può marcare lo spazio percorso, non solo un oggetto manipolato.
*   `{{一気|いっき}}に` ➔ **Modo dell'azione**: il movimento avviene in uno slancio unico.
*   `{{進|すす}}む` ➔ **Avanzamento**: il verbo mette in primo piano il procedere attraverso l'area.
...
## 3. Battaglia e squadra: sonno, potenza e crescita
...
:::example_sentence
jp: >-
  {{結晶|けっしょう}}{{化|か}}した ポケモンは {{強力|きょうりょく}}な わざを {{出|だ}}す。
translation_it: >-
  Il Pokémon cristallizzato usa una mossa potente.
:::

#### 🗺️ Anatomia della frase

*   `{{結晶|けっしょう}}{{化|か}}した ポケモンは` ➔ **Tema in uno stato già attivo**: il modificatore descrive il Pokémon come già cristallizzato.
*   `{{強力|きょうりょく}}な わざを` ➔ **Oggetto qualificato**: `な` collega l'aggettivo nominale a `わざ`, mentre `を` marca la mossa usata.
*   `{{出|だ}}す` ➔ **Azione in battaglia**: la mossa viene messa in campo.
...
*   `{{手|て}}{{加減|かげん}}` ➔ **Controllo della forza**: letteralmente richiama la regolazione della mano, in uso significa trattenersi o andarci piano.
*   `しない` ➔ **Negazione dell'azione**: il parlante rifiuta di trattenersi.
*   `よ` ➔ **Spinta verso l'interlocutore**: la battuta suona assertiva, adatta a una sfida.
...
## 5. Decisioni e conferme: ciò che succede, ciò che scegli, ciò che rendi possibile

I pattern con `こと` e `よう` separano tre tipi di decisione che in gioco compaiono spesso: una situazione stabilita, una scelta personale e un comportamento da mantenere. Subito dopo, una formula di conferma può fermare l'azione e chiederti se vuoi davvero procedere.

### A. Quando la situazione decide: [～ことになる](grammar:grammar-g075-si-decide-che-finire-per)

[～ことになる](grammar:grammar-g075-si-decide-che-finire-per) presenta l'evento come risultato di una regola, di una situazione o di una decisione esterna. Il soggetto può essere coinvolto, ma la frase non insiste sulla sua scelta personale. Per questo in italiano spesso rende bene con "si decide che", "finire per" o "viene stabilito che".
...
*   `ゼロゲートに` ➔ **Destinazione**: il luogo verso cui si andrà.
*   `{{行|い}}くこと` ➔ **Azione nominalizzata**: "andare" diventa il contenuto della decisione.
*   `になった` ➔ **Risultato stabilito**: la situazione è arrivata a quel punto.
...
*   `{{周|まわ}}りを` ➔ **Oggetto controllato**: l'attenzione va all'area circostante.
*   `{{見|み}}ること` ➔ **Azione resa decisione**: il verbo in forma dizionario entra prima di `こと`.
*   `にする` ➔ **Scelta attiva**: il soggetto decide di fare quell'azione.
...
:::example_sentence
jp: >-
  {{危|あぶ}}ない {{場所|ばしょ}}には {{近|ちか}}づかないようにする。
translation_it: >-
  Faccio in modo di non avvicinarmi ai luoghi pericolosi.
:::

#### 🗺️ Anatomia della frase

*   `{{危|あぶ}}ない {{場所|ばしょ}}には` ➔ **Area da trattare con cautela**: `には` mette il luogo come punto rilevante della regola.
...
Le stesse forme si combinano spesso quando l'esplorazione passa a una scelta concreta, poi a una sfida o a un messaggio di sistema:

:::example_sentence
jp: >-
  {{謎|なぞ}}の {{結晶|けっしょう}}{{化|か}}について アドバイスを {{聞|き}}く。
translation_it: >-
...
:::example_sentence
jp: >-
  {{一瞬|いっしゅん}}だけ {{周|まわ}}りを {{見|み}}てから、{{近|ちか}}づくことにする。
translation_it: >-
  Dopo essermi guardato intorno solo un istante, decido di avvicinarmi.
:::

:::example_sentence
jp: >-
  {{広|ひろ}}い {{水上|すいじょう}}では、{{危|あぶ}}ない {{動|うご}}きを しないようにする。
translation_it: >-
  Sull'ampia superficie d'acqua faccio in modo di non fare movimenti pericolosi.
:::

:::example_sentence
jp: >-
  {{強力|きょうりょく}}な ポケモンを {{育|そだ}}てることになった。
translation_it: >-
  Si è deciso di allenare un Pokémon potente.
:::

:::example_sentence
jp: >-
  ねむりの ポケモンは まだ {{動|うご}}けない。
translation_it: >-
  Il Pokémon addormentato non può ancora muoversi.
:::
