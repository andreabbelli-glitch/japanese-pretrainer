...
[{{止|と}}める](term:term-anki-l07-tomeru) è transitivo: qualcuno ferma qualcosa. Questo lo distingue da `{{止|と}}まる`, dove la cosa si ferma da sola o viene descritta come già ferma. Nei menu e nei tutorial questa differenza è operativa: `{{進|しん}}{{化|か}}を {{止|と}}める` non significa "l'evoluzione si ferma", ma "blocchi tu l'evoluzione".

[{{店|みせ}}](term:term-anki-l07-mise) si appoggia spesso a verbi concreti come `{{行|い}}く`, `{{入|はい}}る` e `{{買|か}}う`. La particella ti dice che rapporto hai con il luogo: `{{店|みせ}}に{{行|い}}く` è movimento verso il negozio, `{{店|みせ}}に{{入|はい}}る` è ingresso, `{{店|みせ}}で{{買|か}}う` è acquisto nel punto commerciale. Se la frase contiene ingredienti, strumenti o ricette, il negozio diventa il frame in cui una risorsa entra nel viaggio.
...
- `Bボタンで` -> `で` marca il mezzo: il tasto B è lo strumento con cui compi l'azione.
- `{{進|しん}}{{化|か}}を` -> `を` marca l'oggetto diretto; l'evoluzione è ciò che viene fermato.
- `{{止|と}}める ことが できる` -> `Vることができる` trasforma il verbo in "puoi fare V"; qui la possibilità è un comando pratico, non una descrizione astratta.

> [!NOTE]
> **🧠 Gancio cognitivo:** pensa a [{{止|と}}める](term:term-anki-l07-tomeru) come a una mano sul freno: c'è sempre qualcosa che viene bloccato. Se la frase non mostra un oggetto con `を`, controlla se è sottinteso dal menu o dal contesto.

## 3. Numeri, moltiplicatori e limiti leggibili

[{{倍|ばい}}](term:term-anki-l07-bai) è il pezzo che trasforma un numero in un moltiplicatore. Da solo può indicare "il doppio" o "volte", ma il suo uso più leggibile arriva quando si aggancia a un numero: [{{2倍|にばい}}](term:term-anki-l07-nibai) è "due volte", quindi "doppio". In battaglia questa forma non è decorativa: decide il rapporto tra tipo della mossa, tipo del bersaglio e quantità di danno.

[{{4|よっ}}つ](term:term-anki-l07-yottsu) usa invece il contatore generale `つ`. Non conta Pokémon come farebbe `{{匹|ひき}}` e non conta persone come `{{人|にん}}`: conta "cose" in modo generico. Per questo funziona bene con oggetti astratti o slot di sistema, come le mosse che un Pokémon può conoscere. La lettura è irregolare: `{{4|よっ}}つ`, non `よんつ`.
...
- `くさタイプの わざは` -> il tema sono le mosse di tipo Erba; `の` collega il tipo alla mossa.
- `みずタイプの ポケモンに` -> `に` marca il bersaglio che riceve l'effetto.
- `{{2倍|にばい}}の ダメージを` -> [{{2倍|にばい}}](term:term-anki-l07-nibai) modifica `ダメージ` tramite `の`: non è "due danni", ma "danno raddoppiato".
- `{{与|あた}}える` -> verbo "dare / infliggere"; nel frame di battaglia il soggetto causa danno al bersaglio.

> [!NOTE]
> **⚖️ Contrasto operativo:** [{{倍|ばい}}](term:term-anki-l07-bai) moltiplica, mentre `つ` conta unità generiche. [{{2倍|にばい}}](term:term-anki-l07-nibai) cambia l'intensità di un effetto; [{{4|よっ}}つ](term:term-anki-l07-yottsu) cambia il numero di elementi disponibili.
...
[{{優|やさ}}しい](term:term-anki-l07-yasashii) entra invece nel modo in cui il gioco valuta persone e atteggiamenti. Con il kanji `{{優|やさ}}` indica gentilezza, premura, dolcezza; non va confuso con `{{易|やさ}}しい`, che ha la stessa lettura ma significa "facile". Nei dialoghi dei personaggi la differenza è decisiva: una persona [{{優|やさ}}しい](term:term-anki-l07-yasashii) non è semplice da battere o da capire, è premurosa.
...
- `スター{{団|だん}}には` -> `に` colloca la regola "dentro" il gruppo; `は` mette il Team Star come tema.
- `{{誰|だれ}}もが` -> "chiunque / tutti"; `も` con `{{誰|だれ}}` allarga il soggetto senza eccezioni.
- `{{守|まも}}らなければならない` -> costruzione di obbligo: "deve essere rispettato / bisogna rispettare".
