---
id: lesson-pokemon-scarlet-violet-pokemon-videogame-core-001-pokemon-videogame-core-ui-and-menus
media_id: media-pokemon-scarlet-violet
slug: 001-pokemon-videogame-core-ui-and-menus
title: "UI e menu: salvare, scegliere e gestire la squadra"
order: 10
segment_ref: pokemon-videogame-core
difficulty: custom
status: active
tags: [pokemon, core, ui, menus]
prerequisites: []
summary: >-
...
- `レポートを {{書|か}}く` — salvare registrando lo stato dell'avventura
- `{{手|て}}{{持|も}}ちに {{加|くわ}}わる` — entrare nella squadra attiva
- `ボックスに {{送|おく}}られる` — venire mandato nel box
- `バッグを {{開|ひら}}く` — aprire la borsa / inventario
- `{{目的|もくてき}}{{地|ち}}を {{登録|とうろく}}する` — registrare una destinazione sulla mappa

## Pattern grammaticali chiave

- [～ことができる](grammar:grammar-koto-ga-dekiru) — una funzione è disponibile / puoi fare qualcosa
- [～ようになる](grammar:grammar-you-ni-naru) — una funzione o regola diventa disponibile stabilmente
- [～ておく](grammar:grammar-teoku) — fare in anticipo e lasciare pronto

## Etichette da riconoscere

- `はい` / `いいえ` — conferma o rifiuto nei prompt di scelta
...
## 1. Salvare non è "fare un report": レポート come memoria della sessione
...
La formula più naturale è `レポートを {{書|か}}く`. Il verbo
`{{書|か}}く` significa "scrivere", e qui conserva l'immagine del diario: non
stai solo premendo un tasto, stai scrivendo lo stato dell'avventura nel file di
salvataggio. Subito intorno compariranno spesso `はい`, `いいえ`, `やめる` o
`{{戻|もど}}る`: sono piccole parole di controllo, ma decidono se l'azione va
avanti, si annulla o torna indietro.

:::example_sentence
jp: >-
  レポートを {{書|か}}いてから {{冒険|ぼうけん}}を {{終|お}}えよう。
translation_it: >-
  Salviamo la partita prima di chiudere l'avventura.
:::

#### 🗺️ Anatomia della frase

- `レポートを` ➔ oggetto diretto: `を` marca ciò che viene "scritto", cioè il salvataggio.
...
perfetta, ma aiuta a ricordare perché il verbo naturale è `{{書|か}}く`.
...
[バッグ](term:term-bag) è l'inventario del giocatore: aprirla significa
guardare ciò che possiedi. Dentro la borsa puoi trovare categorie come
[どうぐ](term:term-dougu), parola larga per strumenti usabili, e
[たいせつなもの](term:term-taisetsu-na-mono), la sezione degli oggetti chiave.
La seconda è scritta in hiragana, ma dietro c'è `{{大切|たいせつ}}なもの`:
"cose importanti". In pratica sono oggetti che sbloccano sistemi, movimenti o
...
:::example_sentence
jp: >-
  バッグを {{開|ひら}}いて ボールの {{数|かず}}を {{確認|かくにん}}しよう。
translation_it: >-
  Apriamo la borsa e controlliamo quante Ball abbiamo.
:::

#### 🗺️ Anatomia della frase

- `バッグを {{開|ひら}}いて` ➔ prima azione in sequenza: aprire l'inventario.
- `ボールの {{数|かず}}を` ➔ oggetto del controllo: il numero delle Ball.
- `{{確認|かくにん}}しよう` ➔ volitivo: "controlliamo", tipico di consigli operativi.
...
la "forza" nominalizzata, `を` la rende oggetto, `{{見|み}}る` è vedere:
nella UI, però, questa somma apre il riepilogo con statistiche, tipo, abilità,
mosse e [もちもの](term:term-mochimono).

Da quella zona del menu partono tre verbi molto concreti. [{{並|なら}}び{{替|か}}え](term:term-narabikae)
...
### A. ～ことができる: funzione disponibile adesso

[～ことができる](grammar:grammar-koto-ga-dekiru) prende un verbo, lo nominalizza
con `こと` e aggiunge `ができる`, "è possibile". Nella UI vale quasi sempre
"da questa schermata puoi fare questa azione".

:::example_sentence
jp: >-
  この {{画面|がめん}}で もちものを {{変更|へんこう}}する ことができる。
translation_it: >-
...
- `この {{画面|がめん}}で` ➔ luogo/contesto dell'azione: proprio questa schermata.
- `もちものを {{変更|へんこう}}する` ➔ azione intera: cambiare l'oggetto tenuto.
- `ことができる` ➔ rende l'azione una possibilità concreta, non una descrizione generica.
...
[～ておく](grammar:grammar-teoku) è il pattern della preparazione. Nei menu
significa fare adesso qualcosa che resterà utile dopo: salvare, curare la
[{{手|て}}{{持|も}}ち](term:term-te-mochi), registrare una
[{{目的|もくてき}}{{地|ち}}](term:term-mokutekichi), assegnare un
...
sblocco che cambia stabilmente il dopo; [～ておく](grammar:grammar-teoku) parla
di una preparazione che completi prima che serva davvero.

## Esempi guidati di riepilogo

:::example_sentence
jp: >-
  レポートを {{書|か}}いておくと、あとで {{安心|あんしん}}して
  {{冒険|ぼうけん}}を {{続|つづ}}けられる。
translation_it: >-
  Se salvi in anticipo, dopo puoi continuare l'avventura con più tranquillità.
:::
...
:::example_sentence
jp: >-
  「{{強|つよ}}さを{{見|み}}る」で わざを {{並|なら}}び{{替|か}}えたり
  {{思|おも}}い{{出|だ}}したり できる。
translation_it: >-
  Da "Controlla dati" puoi riordinare le mosse o recuperarne una già appresa.
:::
...
oggetto equipaggiato, mentre [{{目的|もくてき}}{{地|ち}}](term:term-mokutekichi)
e [{{強|つよ}}さを{{見|み}}る](term:term-tsuyosa-o-miru) trasformano mappa e
riepilogo in strumenti di decisione. Quando incontri `できる`, `ようになる` o
`ておく`, il testo non sta solo descrivendo una schermata: ti sta dicendo che
cosa puoi fare, che cosa si è aperto e che cosa conviene lasciare pronto.
