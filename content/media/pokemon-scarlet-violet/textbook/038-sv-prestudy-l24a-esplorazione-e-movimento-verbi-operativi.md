---
id: lesson-pokemon-scarlet-violet-sv-prestudy-038-sv-prestudy-l24a-esplorazione-e-movimento-verbi-operativi
media_id: media-pokemon-scarlet-violet
slug: 038-sv-prestudy-l24a-esplorazione-e-movimento-verbi-operativi
title: SV Pre-study L24A - esplorazione e movimento, verbi operativi
order: 68
segment_ref: scarlet-violet-prestudy
difficulty: custom
status: active
tags:
- pokemon
- scarlet-violet
- prestudy
- phase-a
- lesson-24
prerequisites: []
summary: 'Lessico ed espressioni per esplorazione e movimento, verbi operativi: 22
  carte lessicali attive, 4 formule ricorrenti, 4 pattern grammaticali e 0 etichette
  contestuali da riconoscere.'
---
# Obiettivo

Capire il giapponese che compare in esplorazione e movimento, verbi operativi. Qui contano parole operative, formule di dialogo e strutture che ti dicono subito cosa sta succedendo sullo schermo.

## Contesto

In Scarlet / Violet questi termini ricorrono in menu, messaggi di sistema e dialoghi brevi. Qui trovi soprattutto parole che segnalano azione, stato, scelta e conseguenza nelle scene di esplorazione e movimento.

## Termini chiave

- [覚える](term:term-v432-oboe-ru) — memorizzare / imparare
- [鍛える](term:term-v433-kitae-ru) — allenare / temprare
- [降りる](term:term-v434-ori-ru) — scendere
- [嬉しい](term:term-v435-ureshii) — contento / felice
- [オンライン](term:term-v436-onrain) — online
- [復活](term:term-v437-fukkatsu) — resurrezione / ritorno in campo
- [遅い](term:term-v438-osoi) — lento / tardi
- [門](term:term-v439-mon) — porta / cancello

## Espressioni ricorrenti

- [きみ次第](term:term-e081-kimi-shidai) — dipende da te
- [ここまで来たら](term:term-e082-kokomade-kita-ra) — ormai che siamo arrivati fin qui
- [全力でいこう](term:term-e083-zenryoku-deikou) — andiamo al massimo
- [まだ終わりじゃない](term:term-e084-mada-owari-janai) — non è ancora finita

## Pattern grammaticali chiave

- [～と](grammar:grammar-g085-quando-se-risultato-naturale) — quando / se (risultato naturale)
- [frase relativa (連体修飾)](grammar:grammar-g086-frase-che-modifica-un-nome) — frase che modifica un nome
- [受け身](grammar:grammar-g087-forma-passiva) — forma passiva
- [しか～ない](grammar:grammar-g088-solo-e-nient-altro) — solo e nient'altro

## Spiegazione

Verbi come [覚える](term:term-v432-oboe-ru), [鍛える](term:term-v433-kitae-ru) e [降りる](term:term-v434-ori-ru) indicano azioni molto concrete: memorizzare o imparare, allenare e scendere. Quando compaiono, ti dicono subito quale operazione è possibile o richiesta.

Le espressioni come [きみ次第](term:term-e081-kimi-shidai), [ここまで来たら](term:term-e082-kokomade-kita-ra), [全力でいこう](term:term-e083-zenryoku-deikou) e [まだ終わりじゃない](term:term-e084-mada-owari-janai) dicono chi prende la decisione, quando si passa alla mossa successiva e se la scena spinge a insistere o a chiudere.

Sul piano grammaticale, [〜と](grammar:grammar-g085-quando-se-risultato-naturale) segnala un risultato naturale: fai X e segue Y. La [frase relativa (連体修飾)](grammar:grammar-g086-frase-che-modifica-un-nome) specifica un nome con una frase prima del sostantivo. [受け身](grammar:grammar-g087-forma-passiva) sposta il fuoco su chi subisce l'azione, mentre [しか～ない](grammar:grammar-g088-solo-e-nient-altro) restringe il campo a una sola possibilità.

## Esempi guidati

:::example_sentence
jp: >-
  それは きみ{{次第|しだい}}だよ。
translation_it: >-
  Dipende da te.
:::

:::example_sentence
jp: >-
  ここまで {{来|き}}たら もう {{全力|ぜんりょく}}で いこう。
translation_it: >-
  Ormai che siamo arrivati fin qui, andiamo al massimo.
:::

:::example_sentence
jp: >-
  {{全力|ぜんりょく}}で いこう！
translation_it: >-
  Andiamo al massimo.
:::

:::example_sentence
jp: >-
  まだ {{終|お}}わりじゃない。
translation_it: >-
  Non è ancora finita.
:::

## Nota finale

Leggi prima il verbo o la struttura che muove la scena: 次第 assegna la scelta, と lega azione e risultato, しか〜ない restringe le possibilità, 受け身 sposta il fuoco su chi subisce.
