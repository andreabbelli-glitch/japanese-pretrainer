...
Nei menu di *Pokémon Scarlet & Violet* il giapponese è compatto perché deve stare accanto a pulsanti, icone e finestre di conferma. Una parola breve può dirti se stai collegando il gioco alla rete, pubblicando un dato, scegliendo una sfida o accettando la perdita di qualcosa.

La lettura utile non è tradurre ogni label in isolamento: è capire che tipo di azione la schermata sta preparando. Prima di premere, separa connessione, controllo, stato, bonus e conseguenza.
...
- [{{合計|ごうけい}}](term:term-v612-goukei) — totale / somma complessiva
- [{{大量|たいりょう}}{{発生|はっせい}}](term:term-v613-tairyouhassei) — apparizione massiccia
- [{{入力|にゅうりょく}}](term:term-v614-nyuuryoku) — inserimento di dati

## Espressioni ricorrenti

- [〜してあげましょう](term:term-e094-shiteagemashou) — facciamolo per qualcuno / ci penso io
...
- [{{研究|けんきゅう}}{{室|しつ}}](term:term-v599-kenkyuushitsu) — laboratorio
- [{{表情|ひょうじょう}}](term:term-v603-hyoujou) — espressione del volto
- [どく{{状態|じょうたい}}](term:term-v605-dokujoutai) — stato di avvelenamento
- [{{壁|かべ}}](term:term-v598-kabe) — parete / barriera
- [{{距離|きょり}}](term:term-v606-kyori) — distanza
- [{{山頂|さんちょう}}](term:term-v607-sanchou) — cima della montagna
- [{{不意|ふい}}](term:term-v604-fui) — sorpresa / imprevedibilità
...
*   `ポケポータルに` ➔ **Punto di arrivo digitale** (`に` marca il luogo o servizio a cui ci si collega).
*   `{{接続|せつぞく}}して` ➔ **Azione preparatoria** (prima colleghi il gioco, poi puoi usare o leggere le funzioni online).
...
*   `なくなって` ➔ **Cambio di stato verso l'assenza** (l'elemento precedente passa da "c'è" a "non c'è più").
*   `しまいます` ➔ **Completamento con conseguenza scomoda** (la perdita arriva a termine e non è presentata come neutra).
...
*   `{{入力|にゅうりょく}}したコード` ➔ **Nome modificato da un'azione passata** (`{{入力|にゅうりょく}}した` specifica che il codice è già stato inserito).
*   `によって` ➔ **Criterio variabile** (il risultato dipende dal codice, non è fisso).
...
*   `いっしょに` ➔ **Cornice di compagnia** (l'azione non è solitaria).
*   `{{山頂|さんちょう}}まで` ➔ **Limite di arrivo** (`まで` marca "fino a", quindi la cima è la meta finale).
...
Le schermate diventano più leggibili quando separi azione, stato e conseguenza:
...
:::example_sentence
jp: >-
  {{大量|たいりょう}}{{発生|はっせい}}の{{場所|ばしょ}}と{{距離|きょり}}を{{確認|かくにん}}してみましょう！
translation_it: >-
...
:::example_sentence
jp: >-
  {{公式|こうしき}}シーズンの{{結果|けっか}}を{{公開|こうかい}}しますか？
translation_it: >-
  Vuoi rendere pubblici i risultati della stagione ufficiale?
:::
