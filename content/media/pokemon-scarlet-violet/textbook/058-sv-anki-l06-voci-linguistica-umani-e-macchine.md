...
## 1. Voci, lezioni e nomi: chi parla e da dove arriva l'informazione

I dialoghi scolastici e le conversazioni in città spesso iniziano da informazioni non ancora verificate. [{{噂|うわさ}}](term:term-anki-l06-uwasa) indica proprio una voce che circola: non è un fatto confermato, ma qualcosa che qualcuno ha sentito, ripetuto o collegato a un luogo o a un Pokémon. Per questo si combina naturalmente con `{{聞|き}}く`, quando il personaggio "sente una voce", oppure con `{{流|なが}}れる`, quando la voce "gira" e passa di persona in persona.
...
> [!NOTE]
> **⚖️ Contrasto operativo:** [{{氏|し}}](term:term-anki-l06-shi) non rende automaticamente il parlante più educato in senso caloroso. Spesso fa l'opposto: aumenta la distanza, come in un annuncio, in un rapporto o in una comunicazione istituzionale.

## 2. Azioni e tono: aumento, fame e fiducia

[{{上|あ}}がる](term:term-anki-l06-agaru) è intransitivo: la cosa che sale prende `が`. In una frase di gioco, questo dettaglio è pratico perché ti dice subito dove guardare. Se leggi `こうげきが {{上|あ}}がる`, il soggetto grammaticale non è il giocatore che alza qualcosa, ma la statistica che aumenta. Il verbo transitivo corrispondente, `{{上|あ}}げる`, mette invece in primo piano chi alza o aumenta un oggetto, un livello o un valore.
...
- `これを {{使|つか}}うと` -> `と` crea una condizione stabile: "se / quando usi questo".
- `ポケモンの こうげきが` -> `が` marca ciò che cambia stato: la statistica, non il giocatore.
- `{{上|あ}}がる` -> aumento visto dal lato della cosa che sale.

> [!NOTE]
> **⚖️ Contrasto operativo:** `こうげきを {{上|あ}}げる` direbbe "alzare l'Attacco" con un agente implicito; `こうげきが` [{{上|あ}}がる](term:term-anki-l06-agaru) dice che l'Attacco aumenta come risultato dell'uso.

[{{食|く}}う](term:term-anki-l06-kuu) significa "mangiare", ma il punto non è la nutrizione: è il registro. Rispetto a `{{食|た}}べる`, [{{食|く}}う](term:term-anki-l06-kuu) suona ruvido, diretto, a volte famelico. In una scena con panini o dialoghi informali, il verbo ti fa sentire il corpo del parlato: chi parla abbassa il registro e non sta scegliendo una forma neutra da manuale.
...
- `おれも` -> pronome informale più `も`: "anch'io", con tono personale e diretto.
- `その サンドウィッチ` -> l'oggetto è tematizzato senza `を`, naturale nel parlato ellittico.
- `{{食|く}}う ぞ` -> verbo ruvido più finale assertiva: il parlante annuncia l'azione con energia.

[{{信|しん}}じる](term:term-anki-l06-shinjiru) non è solo "pensare che qualcosa sia vero". Rispetto a `{{思|おも}}う`, entra nel campo della fiducia: credere in una persona, nei propri Pokémon, in una scelta o in ciò che qualcuno dice. Con `を`, il termine prima del verbo diventa il bersaglio della fiducia.
...
> [!NOTE]
> **🧠 Gancio cognitivo:** pensa a [{{信|しん}}じる](term:term-anki-l06-shinjiru) come a un verbo che chiede investimento personale. `{{思|おも}}う` può restare opinione; [{{信|しん}}じる](term:term-anki-l06-shinjiru) mette in gioco fiducia.
...
[{{楽|たの}}しみ](term:term-anki-l06-tanoshimi) è il piacere trasformato in attesa. Deriva da `{{楽|たの}}しい`, ma come nome può indicare "la cosa che aspetto con gioia". La costruzione `{{楽|たの}}しみに している` rende bene "non vedo l'ora": il parlante tiene quell'evento davanti a sé come fonte di entusiasmo.
...
> [!NOTE]
> **⚖️ Contrasto operativo:** [{{楽|たの}}しみ](term:term-anki-l06-tanoshimi) non significa semplicemente "divertente". `{{楽|たの}}しい` descrive una qualità; [{{楽|たの}}しみ](term:term-anki-l06-tanoshimi) può nominare l'attesa stessa.
...
Queste parole si ricombinano bene perché molte frasi del gioco mescolano fonte dell'informazione, registro sociale e rapporto tra persone o tecnologia.
