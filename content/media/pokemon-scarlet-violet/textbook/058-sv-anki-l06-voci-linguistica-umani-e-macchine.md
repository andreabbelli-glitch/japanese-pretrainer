...
- [{{噂|うわさ}}](term:term-anki-l06-uwasa) — voce, diceria
- [{{言語|げんご}}{{学|がく}}](term:term-anki-l06-gengogaku) — linguistica
- [{{氏|し}}](term:term-anki-l06-shi) — signor, titolo formale
- [{{上|あ}}がる](term:term-anki-l06-agaru) — salire, aumentare
- [{{食|く}}う](term:term-anki-l06-kuu) — mangiare, in registro colloquiale o ruvido
- [{{信|しん}}じる](term:term-anki-l06-shinjiru) — credere, fidarsi
- [{{人間|にんげん}}](term:term-anki-l06-ningen) — essere umano
- [{{解散|かいさん}}](term:term-anki-l06-kaisan) — scioglimento di un gruppo, congedo
- [{{楽|たの}}しみ](term:term-anki-l06-tanoshimi) — piacere atteso, cosa che si aspetta con gioia
- [{{機械|きかい}}](term:term-anki-l06-kikai) — macchina, meccanismo

## Espressioni ricorrenti

- [{{噂|うわさ}}](term:term-anki-l06-uwasa)を {{聞|き}}く — sentire una voce, venire a sapere una diceria
- Nが [{{上|あ}}がる](term:term-anki-l06-agaru) — qualcosa sale o aumenta da sé
- Nを [{{信|しん}}じる](term:term-anki-l06-shinjiru) — credere in qualcuno o qualcosa
- [{{楽|たの}}しみ](term:term-anki-l06-tanoshimi)に している — aspettare qualcosa con gioia, non vedere l'ora
...
*   `あの ポケモンの {{噂|うわさ}}` ➔ **Tema informativo** (`の` lega il Pokémon alla voce: il parlante non sta ancora descrivendo il Pokémon, ma ciò che si dice su di lui).
*   `{{本当|ほんとう}} だった` ➔ **Verifica al passato** (`だった` trasforma la diceria in una constatazione: "era vero").
*   `んだ` ➔ **Scoperta spiegata** (finale che dà il tono di realizzazione: il parlante sta capendo o confermando qualcosa davanti all'interlocutore).
...
*   `これを {{使|つか}}うと` ➔ **Condizione stabile** (`と` presenta una relazione quasi automatica: quando usi questo, l'effetto segue).
*   `ポケモンの こうげきが` ➔ **Valore che cambia** (`が` marca la statistica che aumenta, non chi la sta aumentando).
*   `{{上|あ}}がる` ➔ **Aumento intransitivo** (il cambiamento è visto dal lato della cosa che sale).
...
*   `おれは` ➔ **Punto di vista dichiarato** (il parlante mette sé stesso come tema, con tono personale).
...
*   `ポケモンと {{人間|にんげん}}は` ➔ **Coppia tematica** (`と` mette i due gruppi nello stesso quadro; `は` li presenta come tema generale).
*   `{{助|たす}}け{{合|あ}}って` ➔ **Reciprocità** (`{{合|あ}}う` aggiunge "a vicenda": non un aiuto in una sola direzione).
*   `きました` ➔ **Continuità fino a ora** (`Vてくる` mostra un processo che arriva al presente della scena).
...
translation_it: >-
  Non vedo l'ora della prossima lotta in Palestra.
:::

#### 🗺️ Anatomia della frase

*   `{{次|つぎ}}の ジム{{戦|せん}}、` ➔ **Evento messo in primo piano** (la pausa dopo il nome dell'evento lo porta davanti come tema emotivo).
...
*   `タイムマシンは` ➔ **Oggetto specifico della trama** (il katakana nomina la macchina del tempo come entità riconoscibile).
*   `とても {{複雑|ふくざつ}}な` ➔ **Qualità tecnica** (`{{複雑|ふくざつ}}な` è un aggettivo in `な` che modifica il nome successivo).
...
translation_it: >-
  Credo in un futuro in cui Pokémon ed esseri umani si aiutano a vicenda. Non vedo l'ora anche della prossima lotta in Palestra.
:::
