...
- [{{特防|とくぼう}}](term:term-v088-tokubou) — Difesa Speciale
- [どれ](term:term-v091-dore) — quale, fra più opzioni visibili
- [{{自由|じゆう}}に](term:term-v092-jiyuu-ni) — liberamente
- [{{質問|しつもん}}](term:term-v093-shitsumon) — domanda
- [{{始|はじ}}める](term:term-v095-hajime-ru) — iniziare
- [{{近|ちか}}く](term:term-v098-chikaku) — vicino / nei paraggi
- [{{答|こた}}え](term:term-v099-kotae) — risposta

## Espressioni ricorrenti

- [どうしよう](term:term-e017-doushiyou) — che faccio? / e adesso?
- [どうしたの？](term:term-e018-doushitano) — che c'è? / cos'è successo?

## Pattern grammaticali chiave

- [て{{形|けい}}](grammar:grammar-g022-collega-azioni-crea-richieste-e-molte-co) — collega azioni e costruisce richieste
- [ない{{形|けい}}](grammar:grammar-g023-negazione) — nega un'azione o uno stato
- [ます / {{辞書|じしょ}}{{形|けい}}](grammar:grammar-g024-registro-cortese-vs-piano) — contrasta registro cortese e forma piana
...
Il punto da guardare è la particella. Con `を`, la risposta o l'opzione diventa l'oggetto dell'azione; con `の`, la risposta viene collegata alla domanda che la richiede. In una schermata a scelta multipla, questo ti dice se il gioco sta chiedendo "quale uso?", "quale risposta scelgo?" o "a quale domanda sto rispondendo?".
...
*   `どれを` ➔ **opzione scelta come oggetto** (`どれ` seleziona fra più cose visibili; `を` marca ciò che verrà usato).
*   `{{使|つか}}いますか` ➔ **domanda cortese** (`{{使|つか}}う` diventa `{{使|つか}}います`, e `か` trasforma la frase in domanda).

:::example_sentence
jp: >-
  {{質問|しつもん}}の {{答|こた}}えを {{選|えら}}んでください。
translation_it: >-
  Scegli la risposta alla domanda.
:::
...
## 3. Muoversi nello spazio: libertà, vicinanza e compagni
...
:::example_sentence
jp: >-
  {{特防|とくぼう}}が {{高|たか}}い ポケモンです。
translation_it: >-
  È un Pokémon con Difesa Speciale alta.
:::

#### 🗺️ Anatomia della frase

*   `{{特防|とくぼう}}が` ➔ **statistica messa a fuoco**: `が` evidenzia quale parametro viene descritto.
*   `{{高|たか}}い` ➔ **qualità della statistica**: alto, quindi favorevole sul piano difensivo speciale.
*   `ポケモンです` ➔ **identificazione cortese**: la frase classifica il Pokémon, non racconta un'azione.
...
*   `どう` ➔ **modo interrogativo**: "come?".
*   `しよう` ➔ **volitivo di `する`**: "facciamo / farò?", qui come ricerca ansiosa del prossimo passo.
*   `もう {{時間|じかん}}が ない` ➔ **constatazione negativa**: `もう` segnala che ormai la situazione è cambiata; `が ない` dice che il tempo non c'è.
...
*   `どうした` ➔ **che cosa è successo / che hai fatto**: forma passata usata per leggere la situazione presente.
...
:::example_sentence
jp: >-
  コライドンと {{自由|じゆう}}に {{進|すす}}めます。
translation_it: >-
  Puoi avanzare liberamente con Koraidon.
:::
...
jp: >-
  {{近|ちか}}くに ミライドンが います。
translation_it: >-
  Miraidon è qui vicino.
:::

:::example_sentence
jp: >-
  まだ {{始|はじ}}めない。 どれを {{使|つか}}うか {{考|かんが}}えます。
translation_it: >-
  Non inizio ancora. Penso a quale usare.
:::
