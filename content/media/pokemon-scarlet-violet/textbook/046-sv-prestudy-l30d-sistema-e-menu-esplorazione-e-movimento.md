---
id: lesson-pokemon-scarlet-violet-sv-prestudy-046-sv-prestudy-l30d-sistema-e-menu-esplorazione-e-movimento
...
# Menu, stati e movimento: leggere cosa puoi fare adesso

In *Pokémon Scarlet & Violet* molte righe non portano avanti un dialogo lungo: compaiono su schermate compatte, menu, avvisi di movimento o battute rapide degli NPC. Il gioco ti dice se un evento è aperto, se una notifica va nascosta, se una lista cambia ordine, se puoi prendere qualcosa in prestito o se davanti a te c'è un rischio fisico.

Queste parole vanno lette come comandi di orientamento. [{{落下|らっか}}](term:term-v638-rakka), [{{切|き}}りかえる](term:term-v641-kirikaeru), [{{開催中|かいさいちゅう}}](term:term-v644-kaisaichuu) e [{{中止|ちゅうし}}](term:term-v645-chuushi) non sono soltanto glossario: collegano testo e azione. Quando le riconosci, capisci se devi muoverti con cautela, cambiare visualizzazione, controllare uno stato o interpretare il tono di chi parla.

## Termini chiave

- [{{落下|らっか}}](term:term-v638-rakka) — caduta
...
- [{{下降|かこう}}](term:term-v650-kakou) — discesa
- [{{退|しりぞ}}ける](term:term-v651-shirizoke-ru) — respingere / sconfiggere
- [{{勝|か}}ち{{負|ま}}け](term:term-v654-kachimake) — esito vittoria / sconfitta
- [{{全速力|ぜんそくりょく}}](term:term-v657-zensokuryoku) — massima velocità
- [{{緊急事態|きんきゅうじたい}}](term:term-v658-kinkyuujitai) — emergenza
- [{{退場|たいじょう}}](term:term-v659-taijou) — uscita di scena
- [{{憧|あこが}}れ](term:term-v660-akogare) — ammirazione / aspirazione

## Espressioni ricorrenti

- [{{本気|ほんき}}で？](term:term-e103-honki-de) — sul serio?
- [また{{来|き}}てくださいね](term:term-e104-matakitekudasaine) — torna a trovarci, mi raccomando
- [どの ならびに かえますか？](term:term-e105-dono-narabini-kaemasuka) — in quale ordine vuoi riordinare?
...
- [プロフィールアプリ](term:term-v642-purofiiruapuri) — app del profilo
- [{{開催中|かいさいちゅう}}](term:term-v644-kaisaichuu) — in corso
- [{{中止|ちゅうし}}](term:term-v645-chuushi) — interruzione / annullamento
- [{{通知|つうち}}](term:term-v648-tsuuchi) — notifica
...
*   `とくに` ➔ **Focus selettivo** (tra vari rischi possibili, quello che segue è quello da guardare con più attenzione).
...
:::example_sentence
jp: >-
  {{相手|あいて}}を {{退|しりぞ}}けて、{{勝|か}}ち{{負|ま}}けが {{決|き}}まります。
translation_it: >-
  Respingendo l'avversario, si decide l'esito della sfida.
:::

#### 🗺️ Anatomia della frase

*   `{{相手|あいて}}を` ➔ **Bersaglio dell'azione** (`を` marca chi viene respinto).
...
*   `プロフィールアプリの` ➔ **Ambito della funzione** (`の` collega la notifica all'app del profilo).
*   `{{通知|つうち}}を` ➔ **Oggetto dell'azione** (`を` marca ciò su cui interviene il comando).
...
jp: >-
  どの ならびに かえますか？
translation_it: >-
  In quale ordine vuoi riordinare?
:::

#### 🗺️ Anatomia della frase

*   `どの` ➔ **Scelta tra opzioni** (chiede quale criterio o modalità tra più possibilità).
...
*   `{{支援|しえん}}ボードで` ➔ **Luogo/mezzo operativo** (`で` dice dove o tramite che cosa avviene la ricerca).
...
Quando il testo esce dai menu e torna al parlato, poche sillabe possono bastare a dare tono. Qui non leggi solo informazione: leggi sorpresa, cortesia, registro e giudizio.
...
*   `また` ➔ **Ritorno futuro** ("di nuovo": la frase guarda alla prossima visita).
...
### B. Registro, argomento e giudizio
...
*   `{{拙者|せっしゃ}}は` ➔ **Voce marcata** (pronome arcaizzante: prepara un personaggio che parla in modo stilizzato).
*   `{{道具|どうぐ}}について` ➔ **Argomento incorniciato** (`について` dice che la spiegazione verte sugli strumenti).
...
*   [～ばっかり](grammar:grammar-g108-solo-nientaltro-che) non incornicia un argomento, ma giudica o restringe una ripetizione: "sempre e solo X".
...
*   `{{買|か}}い{{物|もの}}ばっかり` ➔ **Ripetizione concentrata** (`ばっかり` restringe il comportamento a "solo acquisti").
*   `していないで` ➔ **Non continuare così** (forma negativa in `-te`: interrompe l'azione in corso prima del comando successivo).
...
:::example_sentence
jp: >-
  {{今|いま}}は {{開催中|かいさいちゅう}} ですが、{{中止|ちゅうし}}に なることも あります。
translation_it: >-
  Ora è in corso, ma può anche essere annullato.
:::
...
:::example_sentence
jp: >-
  {{交換相手|こうかんあいて}}を {{一覧|いちらん}}で {{確認|かくにん}}します。
translation_it: >-
  Controllo il partner di scambio nell'elenco.
:::
...
Quando leggi una schermata compatta, separa sempre stato, azione e voce. [{{開催中|かいさいちゅう}}](term:term-v644-kaisaichuu) e [{{中止|ちゅうし}}](term:term-v645-chuushi) dicono se qualcosa procede, [{{一覧|いちらん}}](term:term-v653-ichiran) e [{{非表示|ひひょうじ}}](term:term-v652-hihyouji) governano ciò che vedi, [{{落下|らっか}}](term:term-v638-rakka) e [{{全速力|ぜんそくりょく}}](term:term-v657-zensokuryoku) trasformano il movimento in decisione, mentre [{{本気|ほんき}}で？](term:term-e103-honki-de), [また{{来|き}}てくださいね](term:term-e104-matakitekudasaine) e [～でござる](grammar:grammar-g106-registro-samurai-comico) colorano il parlato. Il testo diventa più leggibile quando riconosci quale compito sta svolgendo in quel momento.
