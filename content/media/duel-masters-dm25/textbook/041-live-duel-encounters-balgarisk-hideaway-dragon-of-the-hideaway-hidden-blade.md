...
title: "Balgarisk: Ninja Strike e topdeck in due rami"
order: 69
...
summary: >-
  Leggere Balgarisk: Ninja Strike 8, soglia di mana, controllo del topdeck e
  ramo finale tra mettere in campo o mettere in mana tapped.
---

# Balgarisk: Ninja Strike e topdeck in due rami
...
- [{{相手|あいて}}](term:term-opponent) — l'avversario, cioè il lato che fa partire la finestra difensiva.
- [クリーチャー](term:term-creature) — creatura, soggetto degli attacchi e dei blocchi.
- [{{攻撃|こうげき}}](term:term-attack) — attacco, evento che apre il timing.
- [マナゾーン](term:term-mana-zone) — zona mana, sia soglia numerica sia possibile destinazione finale.
- [{{能力|のうりょく}}](term:term-ability) — abilità, qui la specifica abilità di Ninja Strike già usata o non usata.
- [{{山札|やまふだ}}](term:term-deck) — deck, fonte della carta rivelata e destinazione finale dello Shinobi.
- [{{表向|おもてむ}}き](term:term-face-up) — stato face-up, cioè carta resa visibile.
- [コスト](term:term-cost) — costo della carta rivelata.
- [{{持|も}}つ](term:term-motsu) — avere / possedere una proprietà, qui un costo.
- [{{召喚|しょうかん}}](term:term-summon) — evocare.
- [{{出|で}}る](term:term-deru) / [{{出|だ}}す](term:term-dasu) — entrare in campo / mettere in campo.
- [{{置|お}}く](term:term-oku) — collocare una carta in una zona.

## Espressioni ricorrenti

- [コスト](term:term-cost)を[{{支払|しはら}}わずに](term:term-harau)[{{召喚|しょうかん}}](term:term-summon)してもよい — puoi evocare senza pagare il costo.
- [{{山札|やまふだ}}](term:term-deck)の{{上|うえ}}から{{1枚目|いちまいめ}}を[{{表向|おもてむ}}き](term:term-face-up)にする — rendere visibile la prima carta dall'alto del deck.
- [タップ](term:term-tap)して[マナゾーン](term:term-mana-zone)に[{{置|お}}く](term:term-oku) — mettere una carta in mana già tappata.
- [そのターンの{{終|お}}わりに](grammar:grammar-turn-timing) — alla fine di quel turno, non subito.

## Pattern grammaticali chiave

- [〜した{{時|とき}}](grammar:grammar-toki) — quando l'evento appena descritto accade.
- [〜してもよい](grammar:grammar-temoyoi) — possibilità concessa, non obbligo.
- [{{使|つか}}っていなければ](grammar:grammar-te-inakereba) — se non è già stato usato.
- [{{枚数|まいすう}}{{以下|いか}}](grammar:grammar-ika-ijou) — limite massimo basato sul numero di carte.
- [そうでなければ](grammar:grammar-soudenakereba) — se non è così, cioè ramo alternativo.

## Etichette da riconoscere

- [{{裏斬隠裏蒼頭|うらぎりがくれうらそうとう}}バルガリスク](term:term-balgarisk-hideaway-dragon-of-the-hideaway-hidden-blade) — nome proprio della creatura.
- アーマード・ドラゴン / シノビ — razze della carta; `このシノビ` nel testo richiama proprio questa identità.
- ドラゴン・エレメント — categoria richiesta dalla carta rivelata.
- [ニンジャ・ストライク](term:term-ninja-strike) — keyword di ingresso gratuito in una finestra difensiva.
- [W・ブレイカー](term:term-w-breaker) — keyword offensiva che rompe due scudi.
...
*   `{{8枚以上|はちまいいじょう}}あり` non equivale a una frase autonoma. Letto da solo sembra una constatazione sul mana; dentro questa riga è una condizione ancora aperta che porta a `{{召喚|しょうかん}}してもよい`.
*   その{{攻撃中|こうげきちゅう}} non significa "durante il turno". Il referente è l'attacco appena nominato, quindi il controllo su [{{使|つか}}っていなければ](grammar:grammar-te-inakereba) vive dentro quella finestra precisa.

#### 🧠 Gancio cognitivo

Immagina `あり、` come una virgola che tiene il cancello aperto: la soglia di mana è superata, ma il testo non lascia ancora entrare Balgarisk. Serve anche il controllo su `{{使|つか}}っていなければ`; solo dopo arriva `{{召喚|しょうかん}}してもよい`.
...
*   このクリーチャーが[{{出|で}}た](term:term-deru)[{{時|とき}}](grammar:grammar-toki) — **trigger di ingresso**: Balgarisk è il soggetto che entra.
*   [{{山札|やまふだ}}](term:term-deck)の{{上|うえ}}から{{1枚目|いちまいめ}}を[{{表向|おもてむ}}き](term:term-face-up)にしてもよい — **azione opzionale sul topdeck**: puoi mostrare la prima carta dall'alto.
*   それが — **referente rivelato**: `それ` punta alla carta appena resa visibile.
*   {{自分|じぶん}}の[マナゾーン](term:term-mana-zone)にあるカードの[{{枚数|まいすう}}{{以下|いか}}](grammar:grammar-ika-ijou)の[コスト](term:term-cost)を[{{持|も}}つ](term:term-motsu)ドラゴン・エレメントなら — **filtro nominale**: il nome finale è ドラゴン・エレメント, modificato dalla proprietà "avere un costo entro quel numero".
*   [{{出|だ}}す](term:term-dasu) — **ramo positivo**: la carta rivelata entra in campo.
*   [そうでなければ](grammar:grammar-soudenakereba)、[タップ](term:term-tap)して[マナゾーン](term:term-mana-zone)に[{{置|お}}く](term:term-oku) — **ramo negativo**: se il filtro non è vero, la stessa carta viene collocata in mana tappata.
...
Le stesse forme si ricombinano quando devi leggere timing, soglia, filtro e fallback:

:::example_sentence
jp: >-
  [{{相手|あいて}}](term:term-opponent)の[クリーチャー](term:term-creature)が
  [{{攻撃|こうげき}}](term:term-attack)した[{{時|とき}}](grammar:grammar-toki)、
  このシノビを[{{召喚|しょうかん}}](term:term-summon)してもよい。
translation_it: >-
  Quando una creatura avversaria attacca, puoi evocare questo Shinobi.
reveal_mode: sentence
:::

:::example_sentence
jp: >-
  その{{攻撃中|こうげきちゅう}}に[{{能力|のうりょく}}](term:term-ability)を
  {{使|つか}}っていなければ、コストを[{{支払|しはら}}わずに](term:term-harau)
  [{{召喚|しょうかん}}](term:term-summon)してもよい。
translation_it: >-
  Se durante quell'attacco non hai usato l'abilità, puoi evocarla senza pagare
  il costo.
reveal_mode: sentence
:::
...
:::example_sentence
jp: >-
  それが[コスト](term:term-cost){{8以下|はちいか}}の
  ドラゴン・エレメントなら、[{{出|だ}}す](term:term-dasu)。
translation_it: >-
  Se quella carta è un Dragon Element con costo 8 o meno, la metti in campo.
reveal_mode: sentence
:::
...
jp: >-
  ...
  [{{置|お}}く](term:term-oku)。
translation_it: >-
  Altrimenti, la metti tappata nella mana zone.
reveal_mode: sentence
:::
