...
- The fetch order is Kanjium, optional local Shirabe Jisho, Jiten, Wiktionary,
  ...
  consult another source if needed, and only manually save `pitch_accent` in
  `pronunciations.json` when the value is justified; do not count
  `review_required` as a completed fetch.
